...
class UserConfig:
    ...
    use_webgl: bool = field(
        default=True,
        metadata={"help": "If true, will use webgl for graph rendering"},
    )

    bootstrap_theme: str = field(
        default="Litera",
        metadata={"help": "Bootstrap theme from bootswatch"},
    )
    ...
    graph_cmap: str = field(
        ...
        metadata={"help": "Default colormap for graphs"},
    )

    editor_preview: bool = field(
        default=True,
        metadata={"help": "If disabled, will hide preview in the editor page"},
    )
...
class GraphConfig:
    ...
    debug_graph: bool = field(
        default=False,
        metadata={"help": "If true, will render some debug info"},
    )
    ...
    louvain_communities_res: float = field(
        default=1.0,
        metadata={
            "help": ("Parameter for Louvain community detection algorithm")
        },
    )
...
class VaultConfig:
    ...
    ignore_hidden_dirs: bool = field(
        default=True,
        metadata={"help": 'If true, will ignore hidden files, like ".git"'},
    )

    template_dir: str | None = field(
        default=None,
        metadata={"help": "Optional directory to template files"},
    )

    base_config: BaseConfig = field(
        default_factory=BaseConfig,
        metadata={"help": "Base handling config"},
    )

    graph_config: GraphConfig = field(
        default_factory=GraphConfig,
        metadata={"help": "Graph handling config"},
    )

    tasks: list[Task] = field(
        default_factory=list,
        metadata={"help": "List of tasks to be executed in the vault"},
    )
    ...
    short_title: str | None = field(
        default=None,
        metadata={"help": "Short title for the sidebar and page title"},
    )
    ...
    message_list_size: int = field(
        default=100,
        metadata={
            "help": ("This amount of messages will be stored in the vault")
        },
    )
    ...
    message_fetch_limit: int = field(
        default=1,
        metadata={"help": "Take messages only N messages"},
    )

    autosave_time: int = field(
        default=10,
        metadata={"help": "Save documents each autosave_time seconds"},
    )

    autocomplete_max_ngrams: int = field(
        default=10000,
        metadata={"help": "Max ngrams to use in autocomplete"},
    )

    autocomplete_ngram_order: int = field(
        default=4,
        metadata={"help": "Ngram order to build an index in autocomplete"},
    )

    autocomplete_max_ratio_in_key: float = field(
        default=0.1,
        metadata={"help": "Remove frequent ngrams"},
    )
    ...
    max_files_to_upload: int = field(
        default=10,
        metadata={"help": "Maximum files to upload at one time"},
    )
...
class AuthConfig:
    ...
    enabled: bool = field(
        default=False,
        metadata={"help": "If enabled, will use auth"},
    )

    db_path: str = field(
        default="./auth.db",
        metadata={"help": "Path to save auth database"},
    )

    rootname: str = field(
        default="root",
        metadata={"help": "The name for the root user"},
    )

    default_root_pass: str = field(
        default="${oc.env:OBSIFLASK_AUTH,'root'}",
        metadata={"help": "The initial password for the root user"},
    )

    sessions_without_auth: bool = field(
        default=False,
        metadata={
            "help": ('If set, will show "sessions" button even '
    ...
    user_config_dir: str = field(
        default="./user_cfg/",
        metadata={"help": "Path to save user configs"},
    )

    session_cookie_name: str = field(
        default="session",
        metadata={"help": "A cookie name for credential data storage"},
    )
...
class AppConfig:
    vaults: dict[str, VaultConfig] = field(
        metadata={"help": "Config for each vault"}, )
    ...
    log_path: str = field(
        ...
        metadata={"help": "Path to save logs"},
    )

    default_user_config: UserConfig = field(
        default_factory=UserConfig,
        metadata={"help": "User-specific config"},
    )
    ...
    auth: AuthConfig = field(
        ...
        metadata={"help": "Multi-user settings"},
    )
    ...
    shortlink_path: str = field(
        ...
        metadata={"help": ("Path to save shortlink. Must contain '{}'")},
    )


### The following code is generatec by chat-bot just to make a dump for example config
REQUIRED = "<required>"
...
def placeholder_for_field(f):
    ...
    if origin is dict and len(args) == 2 and is_dataclass(args[1]):
        key_placeholder = "<name>"
        value, subcomments = build_example(args[1])
        return {key_placeholder: value}
    ...
    if origin is None and "Optional" in str(f.type):
        return None
...
def build_example(cls):
    ...
    for f in fields(cls):
        ...
        # --- dict[str, Dataclass] ---
        if origin is dict and len(args) == 2 and is_dataclass(args[1]):
            sub_data, sub_comments = build_example(args[1])
            data[f.name] = {KEY_PLACEHOLDER: sub_data}
            comments[f.name] = {KEY_PLACEHOLDER: sub_comments}
            continue

        # --- list[Dataclass] ---
        if origin is list and len(args) == 1 and is_dataclass(args[0]):
            sub_data, sub_comments = build_example(args[0])
            data[f.name] = [sub_data]
            comments[f.name] = [sub_comments]
            continue

        # --- вложенный dataclass ---
        if is_dataclass(f.type):
            sub_data, sub_comments = build_example(f.type)
            data[f.name] = sub_data
            comments[f.name] = sub_comments
            continue
        ...
        if f.default is not MISSING:
            ...
            value = []
        elif "Optional" in str(f.type):
            value = None
        else:
            value = REQUIRED

        data[f.name] = value
        comments[f.name] = f.metadata.get("help")
...
def dump_with_comments(data, comments):
    def recurse(d, c):
        if isinstance(d, dict):
            cm = CommentedMap()
            for k, v in d.items():
                sub_c = c.get(k) if isinstance(c, dict) else None
                cm[k] = recurse(v, sub_c)

                if isinstance(sub_c, str) and sub_c.strip():
                    cm.yaml_set_comment_before_after_key(k, before=sub_c)
            return cm

        if isinstance(d, list):
            cs = CommentedSeq()
            for i, v in enumerate(d):
                sub_c = c[i] if isinstance(c, list) and i < len(c) else None
                cs.append(recurse(v, sub_c))
            return cs

        return d

    return recurse(data, comments)


if __name__ == '__main__':
    data, comments = build_example(AppConfig)
    doc = dump_with_comments(data, comments)

    yaml = YAML()
    yaml.dump(doc, open("example.yaml", "w"))
